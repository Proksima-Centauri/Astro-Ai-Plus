...
from PyQt5.QtGui import QImage, QPixmap, QPainter, QIcon, QColor, QFont, QPolygon, QPolygonF, QPen, QPalette, QRegion, QLinearGradient
from PyQt5.QtCore import Qt, QPoint, QPointF, QRectF, QThread, pyqtSignal, QTimer, QSize, QByteArray, QEvent, qInstallMessageHandler
...
class StarNetDialog(QDialog):
    def __init__(self, parent=None, stride: int = 16, starnet_path: str = None):
        ...
        self.spin_stride.setRange(1, 512)
        self.spin_stride.setValue(int(stride))
        self.spin_stride.setSingleStep(1)
...
class Fly3DDialog(QDialog):
    ...
    def _remove_stars_now(self):
        ...
        if not starnet_path or not os.path.exists(starnet_path):
            QMessageBox.warning(self, "Usun gwiazdy", "Brak poprawnej sciezki do StarNet++.")
            return
...
class AIAssistantPanel(QFrame):
    ...
    def validate_ase_code(self, code: str) -> tuple[bool, str]:
        allowed_commands = {
            ...
            "open.console", "open.menu", "run.magic", "run.starnet", "run.starnet++",
            ...
            "cameraraw", "reset", "resetsliders", "darkon", "darkoff", "models",
            "exit", "quit", "help"
        }
        for raw_line in code.splitlines():
            line = raw_line.strip()
...
class StarNetWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            result = subprocess.run(
                [self.starnet_path, input_path, output_path, "--stride", str(self.stride)],
                cwd=os.path.dirname(self.starnet_path) or None,
                capture_output=True,
                text=True,
                encoding="utf-8",
                errors="replace",
            ...
            output_candidates = self._find_output_candidates(temp_dir, output_path, start_time)
            result_img = None
    ...
    def _find_output_candidates(self, temp_dir, output_path, start_time):
        ...
        search_dirs = [temp_dir, os.path.dirname(self.starnet_path)]
...
def run_starnet_sync_for_image(img: np.ndarray, starnet_path: str, stride: int = 16):
    ...
    try:
        ...
        creationflags = subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0
        result = subprocess.run(
            [starnet_path, input_path, output_path, "--stride", str(int(stride))],
            cwd=os.path.dirname(starnet_path) or None,
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
        ...
        output_candidates = worker._find_output_candidates(temp_dir, output_path, time.time())
        for candidate in output_candidates:
            if os.path.exists(candidate):
...
class Fly3DWorker(QThread):
    ...
    def run(self):
        ...
        if use_starless:
            if not self.starnet_path or not os.path.exists(self.starnet_path):
                self.finished_signal.emit("", "3D FLY requires StarNet++ path for starless mode.")
                return
        ...
        if zone_map is not None:
            ...
            for behavior in behaviors_sorted:
                ...
                if built is None:
                    continue
                layer_image_raw, raw_mask = built
        ...
        try:
            ...
            for frame_idx in range(frame_count):
                ...
                for layer in layers:
                    ...
                    move_x = layer.move_x * ease * depth_gain
                    move_y = layer.move_y * ease * depth_gain
...
class CorrectionDialog(QDialog):
    def __init__(self, camera_raw_panel, hsl_panel,
        ...
        self.btn_apply.clicked.connect(apply_callback)
        ...
        self.btn_ok.clicked.connect(ok_action)
...
class AstroApp(QMainWindow):
    ...
    def run_starnet(self):
        ...
        if not self.starnet_path or not os.path.exists(self.starnet_path):
            self.log("StarNet++ path is not set. Use the Select StarNet++ button first.", "warning")
            return
    ...
    def add_thumbnail(self, operation_name: str, img: np.ndarray = None):
        ...
        parent_id = self.current_history_node_id
    ...
    def _update_thumbnails_view(self):
        ...
        for idx, history_item in enumerate(self.processing_history):
            depth = self._history_depth_for_node(history_item, nodes_by_id, depth_cache)
            thumb_widget = ClickableThumbnailWidget(
                idx,
                history_item,
                selected=(idx == self.selected_thumbnail_index),
            )
            thumb_widget.thumbnail_clicked.connect(self._on_thumbnail_clicked)
            node_id = history_item.get("id") if isinstance(history_item, dict) else None
            if node_id is not None:
                widgets_by_id[node_id] = thumb_widget
            self.thumbnails_layout.addWidget(thumb_widget, depth, idx)
...
class ThumbnailTreeContainer(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for node_id, node in nodes_by_id.items():
            ...
            start = QPoint(parent_rect.center().x(), parent_rect.bottom())
            end = QPoint(child_rect.center().x(), child_rect.top())
            mid_y = (start.y() + end.y()) // 2
            painter.drawLine(start, QPoint(start.x(), mid_y))
            painter.drawLine(QPoint(start.x(), mid_y), QPoint(end.x(), mid_y))
            painter.drawLine(QPoint(end.x(), mid_y), end)
